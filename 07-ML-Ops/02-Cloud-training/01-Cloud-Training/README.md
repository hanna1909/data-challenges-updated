...
🚨 Keep in mind that the `.env` file **should never be stored in Git**.
...
<details>
  ...
  👉 This separation is an indication that we might want to retrieve data from other data sources such as **Cloud Storage** or **Big Query**
</details>
...
## Install `taxifare-model` version `0.0.2`
...
```bash
pip list | grep taxifare-model
# taxifare-model               0.0.2
```
...
**📝 Fill the `LOCAL_DATA_PATH` variable in the `.env` project configuration with `~/.lewagon/mlops/data`**

**📝 Fill the `LOCAL_REGISTRY_PATH` variable in the `.env` project configuration with `~/.lewagon/mlops/training_outputs`**

**🧪 Check the new `LOCAL_DATA_PATH` and `LOCAL_REGISTRY_PATH` values**
```bash
echo $LOCAL_DATA_PATH
...
<details>
  <summary markdown='span'><strong> 💡 Check all the environment variable loaded by <code>direnv</code></strong></summary>
...
📚 Take the time to read the `interface.main.preprocess` function to understand how it retrieves the data chunk by chunk.

### Step 2: `preprocess` saves the preprocessed data

📚 Take the time to read the `interface.main.preprocess` function to understand how it saves the preprocess data.
...
📚 Take the time to read the `interface.main.train` function to understand how it retrieves the preprocessed data chunk by chunk.
...
📚 Let's have a look at the `ml_logic.data.get_chunk` _function_ in order to undertand how it can switch from local to cloud data loading (the `save_chunk` _function_ works similarly for storage).

We provide you with the code of the `data_sources.local_disk` _module_ so you can see how the `get_pandas_chunk` and `save_local_chunk` are working.

✋ Ask for a TA if you need explanations to understand any of the above steps.

### Step 5: `big_query.py` is where to code

At last here is the place where you should work: the `data_sources.big_query` _module_ contains the `get_bq_chunk` and `save_bq_chunk` methods that you need to implement.
